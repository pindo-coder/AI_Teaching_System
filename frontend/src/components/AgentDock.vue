...
import { ArrowDown, Close, MagicStick, RefreshRight, Setting } from '@element-plus/icons-vue'
  ...
  <section v-if="auth.isAuthenticated" class="agent-dock" :class="{ 'is-expanded': expanded }" aria-label="思政 AI 工作台">
    ...
    <transition name="agent-dock-reveal">
      <div v-if="expanded" class="agent-dock-body">
        <div class="agent-dock-guidance"><span><el-icon><MagicStick /></el-icon></span><div><strong>Chat 用于教材问答，Agent 用于推进任务。</strong><p>Agent 会读取当前课程、专题、教学班和任务状态；创建的备课草稿、资料检查或学习清单均可追踪，发布仍由人工确认。</p></div><button type="button" aria-label="关闭 AI 工作台" @click="expanded = false"><el-icon><Close /></el-icon></button></div>
...
<style scoped>
.agent-dock { position: sticky; top: 64px; z-index: 18; margin: 0 28px; color: #21334f; background: rgba(255, 255, 255, .93); border: 1px solid #dfe7f4; border-top: 0; border-radius: 0 0 18px 18px; box-shadow: 0 10px 28px rgba(32, 58, 104, .08); backdrop-filter: blur(16px); }
...
.agent-dock-body { overflow: hidden; background: linear-gradient(180deg, #f7f9fe, #fff 65%); border-top: 1px solid #e7edf7; border-radius: 0 0 18px 18px; }.agent-dock-guidance { display: flex; align-items: flex-start; gap: 10px; padding: 13px 18px; background: linear-gradient(90deg, #f0f4ff, #f9fbff); border-bottom: 1px solid #e3eaf7; }.agent-dock-guidance > span { display: grid; width: 25px; height: 25px; flex: 0 0 auto; place-items: center; color: #fff; background: #5067d9; border-radius: 8px; }.agent-dock-guidance div { min-width: 0; flex: 1; }.agent-dock-guidance strong { color: #2e4670; font-size: 12px; }.agent-dock-guidance p { margin: 3px 0 0; color: #77869c; font-size: 11px; line-height: 1.55; }.agent-dock-guidance button { display: grid; width: 27px; height: 27px; place-items: center; color: #71819a; background: transparent; border: 0; cursor: pointer; }
...
@media (max-width: 760px) { .agent-dock { top: 58px; margin: 0 10px; border-radius: 0 0 14px 14px; }.agent-dock-bar { gap: 8px; padding: 8px 10px; }.agent-dock-brand { min-width: 43px; }.agent-dock-title, .agent-dock-context { display: none; }.agent-dock-actions { flex: 1; justify-content: flex-end; }.agent-dock-actions :deep(.el-button) { padding: 7px 9px; }.agent-dock-guidance { padding: 11px; }.agent-context-candidates { grid-template-columns: 1fr; } }
</style>

...
import { Collection, Delete, EditPen, MagicStick, Promotion, Reading, Search } from '@element-plus/icons-vue'
import { studyApi, type StudyChatMessage, type StudyNote } from '@/api/study'
...
<template>
  <div v-loading="loading">
    ...
    <section class="notes-navigator">
      <div class="notes-navigator-heading"><div><p class="eyebrow">专题索引</p><h2>选择要继续整理的笔记</h2></div><el-input v-model="query" :prefix-icon="Search" clearable placeholder="搜索专题或笔记内容" /></div>
    ...
    <section class="notes-workspace">
      <main class="note-editor-panel">
        <template v-if="selected">
          <header class="note-editor-heading"><div><span>{{ selected.course_name }}</span><h2>{{ selected.chapter_title }}</h2></div><div class="note-editor-actions"><el-button-group><el-button :type="mode === 'edit' ? 'primary' : 'default'" :icon="EditPen" @click="mode = 'edit'">编辑</el-button><el-button :type="mode === 'preview' ? 'primary' : 'default'" @click="mode = 'preview'">预览</el-button></el-button-group><el-button :icon="Reading" @click="router.push(`/courses/${selected.course_id}/chapters/${selected.chapter_id}/review`)">返回专题</el-button></div></header>
      ...
      <aside class="notes-ai-panel">
        <template v-if="selected">
          <header class="notes-ai-heading"><div class="ai-icon"><el-icon :size="21"><MagicStick /></el-icon></div><div><h2>笔记 AI 助手</h2><p>围绕当前专题笔记与教材资料提问</p></div></header>

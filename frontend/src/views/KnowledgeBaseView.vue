...
import {
  ArrowRight, Clock, Collection, Connection, DocumentChecked, Link, Refresh, Search, UploadFilled,
...
function textbookCourseId(document: KnowledgeDocument) { return document.course_id || document.course_ids[0] || null }
function openTextbookLearning(document: KnowledgeDocument) {
  const id = textbookCourseId(document)
  if (!id) return ElMessage.warning('该教材尚未关联课程，暂时无法打开学习正文')
  router.push(`/courses/${id}`)
}
function openTextbookGovernance(document: KnowledgeDocument) {
  const id = textbookCourseId(document)
  if (!id) return ElMessage.warning('该教材尚未关联课程，暂时无法打开版本管理')
  router.push({ path: `/courses/${id}`, query: { panel: 'versions' } })
}
...
<template>
  <div class="material-center-page">
    <header class="material-center-hero" :style="{ backgroundImage: `url(${learningHeroBackground})` }">
      <div class="material-hero-logo"><img :src="logoUrl" alt="思政红芯"></div>
      <div class="material-hero-copy">
        <h1>思政教学资料中心</h1>
        <p>以中央材料校准方向、以教材正文组织学习、以地方材料紧密实践。所有AI引用均来自可核验原文。</p>
        <nav class="material-priority" aria-label="资料分类">
          <button type="button" :class="{ active: activeTab === 'central' }" @click="activeTab = 'central'; statusFilter = ''">中央材料</button>
          <button type="button" :class="{ active: activeTab === 'textbook' }" @click="activeTab = 'textbook'; statusFilter = ''">课本文档</button>
          <button type="button" :class="{ active: activeTab === 'local' }" @click="activeTab = 'local'; statusFilter = ''">地方材料</button>
        </nav>
      </div>
      <div class="material-hero-metrics">
        <article><strong>{{ counts.central }}</strong><span>中央材料</span></article>
        <article><strong>{{ counts.textbook }}</strong><span>课本文档</span></article>
        <article><strong>{{ counts.local }}</strong><span>地方材料</span></article>
      </div>
    </header>
    ...
    <section class="material-command-bar">
      <div class="material-actions">
        ...
        <el-button v-if="activeTab === 'textbook'" type="primary" :icon="DocumentChecked" @click="router.push('/courses')">进入教材学习</el-button>
      </div>
    </section>

    <section class="material-section-heading"><div><p class="eyebrow">{{ tabMeta[activeTab].label }}</p><h2>{{ tabMeta[activeTab].description }}</h2></div><span v-if="activeTab === 'central'">相关性达标后中央材料优先，但不会脱离当前专题</span><span v-else-if="activeTab === 'local'">教师资料默认仅对所选教学班生效</span><span v-else-if="activeTab === 'textbook'">学习入口读取已发布版本，版本、校准和索引在教材右侧管理</span></section>
    ...
    <div v-loading="loading" class="material-list">
      <article v-for="item in filteredDocuments" :key="item.id" class="material-card" :class="`material-card-${item.material_type}`">
        ...
        <div class="material-card-main">
          <div class="material-card-title"><div><h3>{{ item.source_title }}</h3><p>{{ item.material_type === 'textbook' ? courseName(textbookCourseId(item)) : (item.publisher || courseName(item.course_id)) }}<template v-if="item.published_date"> · {{ item.published_date }}</template></p></div><el-tag :type="statusType(item.review_status)">{{ statusLabel(item.review_status) }}</el-tag></div>
          <p class="material-scope-copy">{{ item.applicable_scope || (item.material_type === 'textbook' ? '当前教材版本与专题范围' : '尚未填写适用范围') }}</p>
          <div class="material-chips"><span v-for="tag in item.knowledge_tags" :key="tag">{{ tag }}</span><span v-if="item.material_type === 'textbook' && item.version_label">版本 {{ item.version_label }}</span><span v-if="item.course_ids.length">关联课程 {{ item.course_ids.length }} 个</span><span v-if="item.chapter_ids.length">{{ item.chapter_ids.length }} 个专题</span><span v-if="item.teaching_class_ids.length">{{ item.teaching_class_ids.length }} 个教学班</span><span>原文分块 {{ item.chunk_count }}</span></div>
        </div>
        <div class="material-card-ops">
          <el-button v-if="item.material_type === 'textbook'" link type="primary" :icon="ArrowRight" @click="openTextbookLearning(item)">查看当前教材</el-button>
          <el-button v-if="item.material_type === 'textbook'" link type="primary" @click="openTextbookGovernance(item)">版本与发布</el-button>
...
@media(max-width:1180px){.material-center-hero{grid-template-columns:1fr}.material-command-bar{align-items:stretch;flex-direction:column}.material-actions{flex-wrap:wrap}.material-card{grid-template-columns:100px minmax(0,1fr)}.material-card-ops{grid-column:1/-1;display:flex;flex-wrap:wrap;border-top:1px solid #e6ebf2;border-left:0;padding-top:12px}.batch-mapping-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:720px){.material-center-hero{padding:32px 24px}.material-center-hero h1{font-size:32px}.material-hero-metrics{grid-template-columns:1fr 1fr}.material-command-bar,.material-tabs,.material-actions{align-items:stretch;flex-direction:column}.material-actions .el-input,.material-actions .el-select{width:100%}.material-card{grid-template-columns:1fr}.material-card-marker{height:80px}.material-card-ops{grid-column:auto}.material-section-heading{align-items:flex-start;flex-direction:column;gap:8px}.material-section-heading>span{line-height:1.6}.material-form-grid,.batch-mapping-grid,.batch-default-grid,.batch-progress-card{grid-template-columns:1fr}.batch-preview-toolbar{align-items:stretch;flex-direction:column}}
.material-center-hero { position: relative; width: min(100%, 1120px); max-width: 100%; grid-template-columns: 150px minmax(250px, 1fr) minmax(300px, 380px); align-items: center; min-height: 300px; overflow: hidden; padding: 28px 36px; color: #3d3036; background-color: #fff8f6; background-position: center; background-size: cover; border: 1px solid #f3d9dc; border-radius: 14px; box-shadow: 0 10px 17px rgba(94, 49, 62, .17); }
.material-hero-logo { display: flex; align-items: center; justify-content: center; }
.material-hero-logo img { display: block; width: 150px; height: 165px; object-fit: contain; filter: drop-shadow(0 6px 8px rgba(155, 39, 51, .13)); }
.material-hero-copy { min-width: 0; }
.material-center-hero h1 { margin: 0 0 8px; color: #f1262f; font-size: 32px; line-height: 1.2; }
.material-center-hero p { max-width: 520px; margin: 0; color: #3f3c40; font-size: 15px; font-weight: 550; line-height: 1.65; }
.material-priority { display: flex; align-items: center; gap: 12px; margin-top: 20px; }
.material-priority button { min-width: 116px; height: 38px; padding: 0 16px; color: #604b78; background: rgba(255, 255, 255, .58); border: 1px solid #ff2835; border-radius: 7px; cursor: pointer; font: inherit; font-size: 16px; transition: color .2s, background .2s, box-shadow .2s, transform .2s; }
.material-priority button:hover, .material-priority button.active { color: #d91f2c; background: rgba(255, 250, 249, .9); box-shadow: 0 6px 12px rgba(225, 48, 59, .14); transform: translateY(-1px); }
.material-hero-metrics { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); align-content: center; gap: 12px; }
.material-hero-metrics article { display: grid; place-items: center; min-height: 98px; padding: 14px 8px; background: rgba(255, 220, 219, .82); border: 1px solid rgba(255, 255, 255, .72); border-radius: 12px; box-shadow: 0 6px 9px rgba(88, 48, 59, .2); }
.material-hero-metrics strong { color: #ee242d; font-size: 34px; line-height: 1; }
.material-hero-metrics span { margin-top: 9px; color: #777276; font-size: 15px; }
@media(max-width:1180px) { .material-center-hero { width: 100%; grid-template-columns: 1fr; min-height: 0; } .material-hero-logo { justify-content: flex-start; } }
@media(max-width:720px) { .material-center-hero { gap: 16px; padding: 22px 18px; } .material-hero-logo img { width: 100px; height: 110px; } .material-center-hero h1 { font-size: 25px; } .material-center-hero p { font-size: 13px; } .material-priority { align-items: stretch; flex-direction: column; gap: 8px; } .material-priority button { width: 100%; height: 36px; font-size: 15px; } .material-hero-metrics { grid-template-columns: 1fr 1fr; gap: 8px; } .material-hero-metrics article { min-height: 78px; } .material-hero-metrics strong { font-size: 28px; } .material-hero-metrics span { margin-top: 5px; font-size: 13px; } }
.material-command-bar { padding: 0; background: transparent; border: 0; border-radius: 0; box-shadow: none; }
</style>
